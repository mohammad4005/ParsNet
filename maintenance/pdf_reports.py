...
def build_report_pdf(report):
    ...
    body_style = ParagraphStyle("FaBody", parent=styles["BodyText"], fontName="Persian", fontSize=8.5, leading=14, textColor=NAVY, alignment=TA_RIGHT)
    ...
    summary_table = table(summary, [75 * mm] * 3)
    ...
    story += [table(group_data, [32*mm, 37*mm, 37*mm, 37*mm, 37*mm, 55*mm]), Spacer(1, 4 * mm)]
    ...
    if report["show_overdue"]:
        ...
        story += [table(overdue_data, [25*mm, 36*mm, 43*mm, 55*mm, 76*mm]), Spacer(1, 4 * mm)]
    ...
    if report["show_completed"]:
        ...
        story += [table(completed_data, [42*mm, 42*mm, 42*mm, 52*mm, 57*mm]), Spacer(1, 4 * mm)]
    ...
    story.append(table(order_data, [25*mm, 25*mm, 38*mm, 40*mm, 50*mm, 57*mm]))
